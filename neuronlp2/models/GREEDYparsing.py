...
class NewStackPtrNet(nn.Module):
	...
	#grand_parents = [[0, 0, 0] for _ in range(beam)] if self.grandPar else None#Los tenemos que asignar a 0 porque -1 da error
        grand_parents = [[0,0] for _ in range(beam)] if self.grandPar else None
        siblings = [[0] for _ in range(beam)] if self.sibling else None #
	...
	stop_now=False#Solo vale para greedy
	...
	for t in range(num_step):
		...
		if stop[base_id] == True:
			stop_now=True#Solo vale para greedy
			if debug: print 'PATH' ,cc, ' ya detenido, pero seguimos actualizando'
		...
		if stop[base_id] == False:new_stacked_types[i, t] = hyp_types[i]
